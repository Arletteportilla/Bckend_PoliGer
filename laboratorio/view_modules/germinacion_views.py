...
class GerminacionViewSet(BaseServiceViewSet, ErrorHandlerMixin, SearchMixin):
    ...
    def _generate_simple_pdf(self, user, germinaciones, search=""):
        ...
        try:
            ...
            filename = f"mis_germinaciones_{user.username}{search_text}.pdf"
            ...
            title = f"Mis Germinaciones - {user.first_name} {user.last_name}".strip()
            ...
            p.drawString(50, y_position - 15, f"Total de germinaciones: {len(germinaciones)}")
            ...
            p.drawString(150, y_position, "Especie")
            p.drawString(280, y_position, "Fecha Siembra")
            p.drawString(380, y_position, "Estado")
            p.drawString(450, y_position, "Cápsulas")
            ...
            for germ in germinaciones:
                ...
                p.drawString(150, y_position, str(germ.especie_variedad or '')[:20])
                p.drawString(280, y_position, str(germ.fecha_siembra or '')[:10])
                p.drawString(380, y_position, str(germ.estado_capsula or '')[:10])
                p.drawString(450, y_position, str(germ.no_capsulas or '0'))
